...
/**
 * 工具统一返回值类型（自 harness.tool 包，替换 spring-ai 时代的 agentic.tool.ToolResult）。
 * <p>
...
public class ToolResult {
    ...
    private ToolResult(@NonNull Status status, @Nullable String message,
                       @Nullable Map<String, Object> data, @Nullable String rawOutput) {
        this.status = status;
    ...
    public static ToolResult toolNotFound(String toolName, Set<String> availableTools) {
        ...
        return new ToolResult(Status.ERROR,
                "工具 '" + toolName + "' 不存在或当前智能体无权使用。可用工具: "
