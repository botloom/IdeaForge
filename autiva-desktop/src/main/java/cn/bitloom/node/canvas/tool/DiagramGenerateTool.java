 ...
 */
@Slf4j
public class DiagramGenerateTool extends AbstractTool<DiagramGenerateTool.Input> {

    private static final String DESCRIPTION = "根据文字描述生成图表元素列表。"
        + "支持流程图、架构图、UML类图、时序图等。"
        + "返回JSON格式的元素数组，每个元素包含type、x、y、width、height、text等字段。"
        + "元素类型支持：rectangle、ellipse、diamond、arrow、line、text。"
        + "布局时请合理安排坐标，确保图表清晰可读。";

    public DiagramGenerateTool() {
        super("DiagramGenerate", DESCRIPTION, Input.class);
    }
    ...
    public ToolResult execute(Input input, ToolContext context) {
        ...
        String diagramType = input.diagramType();
    ...
    public static class Builder {
        ...
        public DiagramGenerateTool build() {
            return new DiagramGenerateTool();
        }
    }
}
