...
/**
 * 文件变更快照 Hook — 在 Write/Edit 真正执行前把原文件内容按轮落盘，为「撤回按钮」提供数据源。
 * <p>
 * order=12：排在 PermissionHook(10) 之后，被审批拦截的调用不会走到这里，快照只记录真正执行的修改。
 * <p>
 * 快照布局（会话目录下）：
 * <pre>
 ...
 */
@Slf4j
public class FileChangeRecorderHook implements IAgentHook {

    private static final Set<String> SNAPSHOT_TOOLS = Set.of("Write", "Edit");

    @Override
    public String name() {
        return "FileChangeRecorderHook";
    }
