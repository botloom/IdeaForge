...
import cn.bitloom.agentic.goal.GoalJudge;
...
/**
 * 目标闭环拦截器（原 GoalJudgeHook 的 LoopInterceptor 形态，
 * 对标 learn-claude-code s17 Goal Loop）。
 *
 * <p>「没有新 tool_use 只说明本轮结束，不能证明目标达成」。本拦截器挂
 * {@code afterRound}（AgentLoop 在最终响应生成后回调，即本轮无工具调用、
 * LLM 认为已完成时），触发独立判断器（{@link GoalJudge}，
 * 另一次轻量模型调用、零工具）复核目标：
 *
 * <ul>
 ...
 */
@Slf4j
public class GoalJudgeInterceptor implements LoopInterceptor {
    ...
    private GoalJudgeInterceptor(GoalManager goalManager, ISessionManager sessionManager, GoalJudge judge,
    ...
    @Override
    public String name() {
        return "GoalJudgeInterceptor";
    }

    @Override
    public int order() {
        return 30; // PermissionInterceptor(10)/TodoReminderInterceptor(20) 之后，MemoryExtractionInterceptor(40) 之前
    }

    @Override
    public void afterRound(LoopContext ctx) {
        if (ctx == null || ctx.sessionId() == null || ctx.branch() != null) {
            return; // 仅主智能体
        }
    ...
    private void evaluate(String sessionId, GoalState state) {
        ...
        // 自动续轮：reason 作为续行消息（不落盘，作为下一轮输入由 SessionMemoryInterceptor 持久化）
    ...
    public static class Builder {
        ...
        public GoalJudgeInterceptor build() {
            if (goalManager == null || sessionManager == null || chatModel == null) {
                throw new IllegalStateException("goalManager/sessionManager/chatModel 均不可为空");
            }
            return new GoalJudgeInterceptor(goalManager, sessionManager, new GoalJudge(chatModel), listener);
        }
    }
}
