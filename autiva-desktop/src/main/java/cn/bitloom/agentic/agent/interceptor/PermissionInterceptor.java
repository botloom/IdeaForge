...
/**
 * 权限控制拦截器（原 PermissionHook 的 LoopInterceptor 形态）—
 * 在工具执行前统一拦截需要审批的操作。
 *
 ...
 */
@Slf4j
public class PermissionInterceptor implements LoopInterceptor {

    private final List<ToolApprovalStrategy> approvalStrategies;
    private final Map<String, ToolApprovalStrategy> strategyByTool;

    public PermissionInterceptor(List<ToolApprovalStrategy> approvalStrategies) {
    ...
    public ToolCallDecision beforeToolCall(String toolName, String input, ToolContext context) {
        ...
        if (blockReason != null) {
            log.info("[PermissionInterceptor] 工具调用被拦截: tool={}, reason={}", toolName, blockReason);
