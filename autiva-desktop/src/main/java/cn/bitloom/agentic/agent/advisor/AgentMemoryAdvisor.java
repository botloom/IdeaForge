...
public class AgentMemoryAdvisor implements BaseChatMemoryAdvisor {
    ...
    private AgentMemoryAdvisor(int order, String memorySystemPrompt, List<ToolCallback> memoryToolCallbacks, BiPredicate<ChatClientRequest, Instant> memoryConsolidationTrigger) {
        this.order = order;
        ...
        if (chatClientRequest.prompt().getOptions() instanceof ToolCallingChatOptions toolOptions) {
            ...
            Prompt augPrompt = chatClientRequest.prompt()
                    .augmentSystemMessage(
                            chatClientRequest.prompt().getSystemMessage().getText() + System.lineSeparator()
                                    + this.memorySystemPrompt + System.lineSeparator()
                                    + (this.memoryConsolidationTrigger.test(chatClientRequest, Instant.now()) ? "<system-reminder>整合长期记忆：总结并移除冗余信息。</system-reminder>" : "") + System.lineSeparator()
                    );
            ...
            toolOptionsCopy = toolOptionsCopy.mutate()
