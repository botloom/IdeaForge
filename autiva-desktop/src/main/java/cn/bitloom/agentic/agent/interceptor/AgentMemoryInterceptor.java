...
import cn.bitloom.harness.tool.JsonSchemaGenerator;
...
/**
 * 自动记忆工具拦截器（原 AgentMemoryAdvisor 的 LoopInterceptor 形态），
 * 为智能体提供长期记忆管理能力。
 *
 * <p>注入记忆工具到智能体的工具列表中，允许智能体通过工具调用管理长期记忆。
 * 支持记忆整合触发器，在合适时机注入整合提醒。
 *
 * <p>原 Advisor 语义为每用户消息注入一次；新 AgentLoop 中 beforeModelCall 在
 * 工具循环中每轮模型调用都会触发，因此用 ctx 标记（{@link #CTX_INJECTED}）
 * 保证一次 run 内只注入一次。
 *
 * <p><strong>工具注册形态变化：</strong>原 Advisor 把 spring-ai ToolCallback 追加到
 * ToolCallingChatOptions；新 AgentLoop 的工具执行统一走 ToolExecutor 注册表，
 * 本拦截器在 beforeModelCall 中按名去重追加 ToolSpec（模型可见），执行侧回调经
 * {@link #memoryToolCallbacks()} 暴露，由装配方注册进 Agent 的工具列表。
 */
public class AgentMemoryInterceptor implements LoopInterceptor {

    /** ctx 标记：记忆上下文已注入（每用户消息一次） */
    static final String CTX_INJECTED = "agentMemory.injected";
    ...
    /** 记忆工具（harness 形态回调），供装配方注册到 Agent 工具列表（执行侧） */
    private final List<ToolCallback> memoryToolCallbacks;

    private final BiPredicate<LoopContext, Instant> memoryConsolidationTrigger;

    private AgentMemoryInterceptor(int order, String memorySystemPrompt, List<ToolCallback> memoryToolCallbacks,
            BiPredicate<LoopContext, Instant> memoryConsolidationTrigger) {
        this.order = order;
    ...
    /** 记忆工具回调列表：装配方应将其注册进 Agent 的工具列表以获得执行能力。 */
    public List<ToolCallback> memoryToolCallbacks() {
    ...
    public ChatRequest beforeModelCall(ChatRequest request, LoopContext ctx) {
        ...
        try {
            ...
            // 1. system 消息追加记忆说明 + 整合提醒（触发器命中时）
            String reminder = this.memoryConsolidationTrigger.test(ctx, Instant.now())
                    ? "<system-reminder>整合长期记忆：总结并移除冗余信息。</system-reminder>"
                    : "";
            String injection = this.memorySystemPrompt + System.lineSeparator() + reminder + System.lineSeparator();
            var messages = new ArrayList<>(request.messages());
            EnvironmentContextInterceptor.injectIntoSystemMessage(messages, injection);

            // 2. 工具注入：按名去重追加记忆工具规格（模型可见；执行侧经 memoryToolCallbacks() 注册）
            List<ToolSpec> tools = new ArrayList<>(request.tools());
            Set<String> existingNames = tools.stream().map(ToolSpec::name).collect(Collectors.toSet());
            this.memoryToolCallbacks.stream()
                    .filter(tc -> !existingNames.contains(tc.definition().name()))
                    .forEach(tc -> {
                        ToolDefinition d = tc.definition();
                        tools.add(ToolSpec.of(d.name(), d.description(), d.inputSchema()));
                    });

            return request.withMessages(messages).withTools(tools);
        } catch (Exception e) {
            return request;
        }
    }
    ...
    public static final class Builder {
        ...
        private BiPredicate<LoopContext, Instant> memoryConsolidationTrigger = (ctx, instant) -> false;
        ...
        public Builder memoryConsolidationTrigger(BiPredicate<LoopContext, Instant> memoryConsolidationTrigger) {
            Assert.notNull(memoryConsolidationTrigger, "Memory consolidation trigger must not be null");
            this.memoryConsolidationTrigger = memoryConsolidationTrigger;
            return this;
        }

        public AgentMemoryInterceptor build() {
            ...
            return new AgentMemoryInterceptor(this.order, memorySystemPromptText, memoryToolCallbacks,
